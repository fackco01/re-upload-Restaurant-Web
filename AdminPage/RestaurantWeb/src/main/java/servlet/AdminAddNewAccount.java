...
public class AdminAddNewAccount extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (duplicationName(username, ls)) {
            ...
            d.NewAcc(autoIncreaseID(ls), username, password, fullname, role, true);
            System.out.println("ADD ACCOUNT: " + username + " - " + password + " - " + fullname + " - " + roleID);
            url = "AdminHomePage";
        }
    ...
    boolean duplicationName(String name, ArrayList<Account> ls) {
        for (int i = 0; i < ls.size(); i++) {
            if (ls.get(i).getAccID().equalsIgnoreCase(name)) {
                return true;
            }
        }
        return false;
